...
def _load_dataset() -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load Amazon dataset: {exc}") from exc

    required_columns = {"product_name", "platform", "price", "url"}
    missing = required_columns - set(df.columns)
    if missing:
        raise RuntimeError(f"Amazon dataset missing columns: {missing}")
...
async def health() -> dict:
    return {"status": "ok", "service": "scraper-amazon"}
